...
class Ajax extends Admin_Controller
{
    ...
    public function getClassByBranch()
    {
        ...
        $branch_id = $this->application_model->get_branch_id();
        if (!empty($branch_id)) {
            $classes = $this->db->select('id,name')->order_by('name','ASC')->where('branch_id', $branch_id)->get('class')->result_array();
            if (count($classes)) {
                $html .= "<option value=''>" . translate('select') . "</option>";
    ...
    public function getStudentByBatch()
    {
        ...
        if (!empty($class_id)) {
            ...
            $html .= '<option value="">' . translate('select_batch_first') . '</option>';
        }
        echo $html;
    }
