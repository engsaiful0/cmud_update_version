...
class Student extends Admin_Controller
{
    ...
    public function script_run()
    {
        $enroll = $this->db->select('*')->from('enroll')->get()->result();
        foreach ($enroll as  $value) {
            $this->db->where('id', $value->student_id)->update('student', array('roll' => $value->roll));
        }
    }
    ...
    public function view()
    {
        ...
        $search_by_anything = $this->input->post('search_by_anything') ?: '';
        
        $sectionID = '';

        // Get current page number from URL
        $page = ($this->uri->segment(3)) ? (int) $this->uri->segment(3) : 1; // Ensure it's an integer
        $limit = 30;
        $start = ($page - 1) * $limit; // Correct offset calculation

        // Pagination configuration
        $this->load->library('pagination');
        $config['base_url'] = base_url() . "student/view/";
        $config['total_rows'] = $this->application_model->count_students($search_by_anything,$roll,$classID, $sectionID, $branchID, $student_id);
        $config['per_page'] = $limit;
        $config['uri_segment'] = 3;
        $config['use_page_numbers'] = TRUE; // Ensure correct page numbers
        $config['num_links'] = 5;
        ...
        $this->data['students'] = $this->application_model->getStudentListByClassSectionForView(
            $search_by_anything,
            $roll,
            $classID,
            $sectionID,
    ...
    public function profile($id = '')
    {
        ...
        if (isset($_POST['update'])) {
            ...
            $post = $this->input->post();
            //save all student information in the database file
            $studentData = $this->student_model->update($post, $getBranch, $id);
            $studentID = $studentData['student_id'];
    ...
    public function course_price_load()
    {
        ...
        if ($query) { // Check if a row is returned
            echo $query->fee;
        } else {
            echo "No record found with ID: " . $subject_id;
        }
    }
    ...
    public function all_students()
    {
        ...
        $this->data['pagination'] = $this->pagination->create_links();
        $this->data['title'] = translate('all_students');
        $this->data['branch_id'] = $branchID;
        $this->data['sub_page'] = 'student/all_students';
        $this->data['main_menu'] = '';
    ...
    public function quickDetails()
    {
        ...
        $data['religion'] = ($row->religion == 0) ? 'Islam' : (($row->religion == 1) ? 'Hinduism' : (($row->religion == 2) ? 'Buddhism' : (($row->religion == 3) ? 'Christianity' : 'N/A')));
