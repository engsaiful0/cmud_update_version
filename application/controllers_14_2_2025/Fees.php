...
class Fees extends Admin_Controller
{
    ...
    public function invoice_list()
    {
        ...
        if ($this->input->post('search')) {
            ...
            $this->data['invoicelist'] = $this->fees_model->getInvoiceList($this->data['class_id'], $this->data['student_id'], $branchID);
        }
        $this->data['branch_id'] = $branchID;
    ...
    public function payment_list()
    {
        ...
        if ($this->input->post('search')) {
            ...
            $this->data['section_id'] = $this->input->post('section_id');
            $this->data['student_id'] = $this->input->post('student_id');
            $this->data['payments'] = $this->fees_model->getAllPayments($this->data['class_id'], $this->data['student_id'], $branchID);
        } else {
    ...
    public function fee_edit($id = '')
    {
        ...
        $this->data['fee'] = $this->fees_model->getFeeById($id);
        $this->data['title'] = translate('fees_type');
        $this->data['sub_page'] = 'fees/fee_edit';
        $this->data['main_menu'] = 'fees/payment_list';
        $this->load->view('layout/index', $this->data);
    }
    public function fee_edit_save()
    {
        ...
        if ($this->form_validation->run() !== false) {
            ...
            $fee_payment_history_id = $this->input->post('fee_payment_history_id');
            $arrayFees = array(
                'student_id' => $student_id,
                'collect_by' => get_loggedin_user_id(),
            ...
            $this->db->where('id', $fee_payment_history_id)->update('fee_payment_history', $arrayFees);
            set_alert('success', translate('information_has_been_updated_successfully'));
            $array = array('status' => 'success');
        } else {
            $error = $this->form_validation->error_array();
    ...
    public function batch_wise_student_fees_report()
    {
        if (!get_permission('fees_reports', 'is_view')) {
            access_denied();
        }
        ...
        $this->data['sub_page'] = 'fees/batch_wise_student_fees_report';
        $this->data['main_menu'] = 'fees_repots';
        $this->data['headerelements'] = array(
            'css' => array(
                'vendor/daterangepicker/daterangepicker.css',
    ...
    public function printFeesInvoice()
    {
        if ($_POST) {
            ...
            foreach ($record_array as $key => $value) {
                $deposit = $this->fees_model->getStudentFeeDeposit($value->studentId, '');
