...
class Accounting_model extends MY_Model
{
    ...
    public function saveVoucher($data)
    {
        ...
        $cbal = $qbal['balance'];
        $bkash_transaction_number = $data['bkash_transaction_number'];
        $rocket_transaction_number = $data['rocket_transaction_number'];
        $nogod_transaction_number = $data['nogod_transaction_number'];
        ...
        $insertTransaction = array(
            ...
            'ref' => $ref_no,
            'roll' => $roll,
            'bkash_transaction_number' => $bkash_transaction_number,
            'rocket_transaction_number' => $rocket_transaction_number,
            'nogod_transaction_number' => $nogod_transaction_number,
    ...
    public function getHeadWiseIncomeExpenseRepots($branchID, $start = '', $end = '', $voucher_head_id = '', $type = '')
    {
        $this->db->select('transactions.*,accounts.name as ac_name,voucher_head.name as v_head,payment_types.name as via_name');
        $this->db->from('transactions');
        $this->db->join('accounts', 'accounts.id = transactions.account_id', 'left');
        ...
        $this->db->where('transactions.voucher_head_id', $voucher_head_id);
        $this->db->where('transactions.branch_id', $branchID);
        $this->db->where('transactions.date >=', $start);
        $this->db->where('transactions.date <=', $end);
        $this->db->order_by('transactions.id', 'ASC');
    ...
    public function get_incomevsexpense($branchID, $start = '', $end = '')
    {
        ...
        transactions ON transactions.voucher_head_id = voucher_head.id WHERE transactions.date >= " . $this->db->escape($start) .
            " AND transactions.date <= " . $this->db->escape($end) . " AND transactions.branch_id = " . $this->db->escape($branchID) . " GROUP BY transactions.voucher_head_id ORDER BY voucher_head.serial_no ASC";
