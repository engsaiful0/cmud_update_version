...
class Application_model extends CI_Model
{
    ...
    public function count_students($search_by_anything, $roll = '', $classID = '', $sectionID = '', $branchID = '', $student_id = '')
    {
        ...
        if ($student_id != '') {
            ...
            if ($sectionID != '') {
                $this->db->where('s.section_id', $sectionID);
            }
        }
        ...
        // Add condition for "search by anything"
        if ($search_by_anything != '') {
            $this->db->group_start(); // Start grouping conditions
            $this->db->like("s.first_name", $search_by_anything); // Search by fullname
            $this->db->join("subject as sub", "s.subject_id = sub.id", "left");
            $this->db->or_like("s.register_no", $search_by_anything);  // Search by register_no
            $this->db->or_like("sub.name", $search_by_anything);  // Search by subject name
            $this->db->or_like("s.email", $search_by_anything);  // Search by email
            $this->db->or_like("s.mobileno", $search_by_anything);  // Search by mobile number
            $this->db->or_like("s.roll", $search_by_anything);  // Search by roll number
            $this->db->or_like("s.bmdc_reg_no", $search_by_anything);  // Search by bmdc_reg_no
            $this->db->or_like("s.blood_group", $search_by_anything); // Search by blood group
            $this->db->or_like("c.name", $search_by_anything); // Search by class_name (class table)
            $this->db->group_end(); // End the grouped conditions
        }
    ...
    public function count_students_backup($search_by_anything, $roll = '', $classID = '', $sectionID = '', $branchID = '', $student_id = '')
    {
        $branchID = $this->session->userdata('branch_id');
    ...
    public function getStudentListByClassSectionForView($search_by_anything = '', $roll = '', $classID = '', $sectionID = '', $branchID = '', $student_id = '', $deactivate = false, $rollOrder = false, $limit = 30, $start = 0)
    {
        $branchID = $this->session->userdata('branch_id');
        // die($branchID);

        $this->db->select("s.id as student_id, sub.name as subject_name, s.is_online_offline, s.roll, s.photo, 
    CONCAT_WS(' ', s.first_name, s.last_name) as fullname, s.register_no, s.email, s.mobileno, 
    s.blood_group, s.birthday, s.admission_date, c.name as class_name");
        ...
        // If the "search by anything" is provided, use LIKE to search across multiple fields
        if ($search_by_anything != '') {
            $this->db->group_start(); // Start grouping conditions
            $this->db->like("s.first_name", $search_by_anything); // Search by fullname
            $this->db->or_like("s.register_no", $search_by_anything);  // Search by register_no
        ...
        if ($roll != '') {
            ...
            $this->db->limit($limit, $start); // Make sure $start is correctly passed
        }

        return $this->db->get()->result_array();
    }

    public function getStudentListByClassSectionForView_backup($search_by_anything = '', $roll = '', $classID = '', $sectionID = '', $branchID = '', $student_id = '', $deactivate = false, $rollOrder = false, $limit = 30, $start = 0)
    {
        $branchID = $this->session->userdata('branch_id');
        // die($branchID);
