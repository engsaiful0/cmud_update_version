...
class Fees_model extends MY_Model
{
    ...
    {
        $sql = "SELECT `student`.`register_no`,`student`.`admission_date`,`student`.`id` as `student_id`, `student`.`first_name`, `student`.`gender`, `subject`.`fee` as `amount`, `subject`.`name` FROM `student` LEFT JOIN
    ...
    {
        $sqlDeposit = "SELECT IFNULL(SUM(`amount`), '0.00') as `total_amount`, IFNULL(SUM(`discount`), '0.00') as `total_discount`, IFNULL(SUM(`fine`), '0.00') as `total_fine` FROM `fee_payment_history` WHERE `student_id` = " . $this->db->escape($student_id) . " AND `financial_year_id` = " . $this->db->escape($financial_year_id);
