...
class Fees_model extends MY_Model
{
    ...
    public function getInvoiceStatus($studentID = '', $roll = '')
    {
        ...
        $student = $this->db->where('id', $studentID)->get('student')->row();
        $subject = $this->db->where('id', $student->subject_id)->get('subject')->row();
        $invNo = $student->register_no;
        $totalFee = $subject->fee;

        if ($paid['amount']+$deposit_through_office_accounting_value == 0) {
            $status = 'unpaid';
        } elseif ($totalFee == ($paid['amount'] + $paid['discount']+$deposit_through_office_accounting_value)) {
            $status = 'total';
        } elseif ($paid['amount']+$deposit_through_office_accounting_value > 1) {
            $status = 'partly';
        }
    ...
    {
        $sql = "SELECT `student`.`register_no`,`student`.`financial_year_id`,`student`.`course_price`,`student`.`course_price_discount`,`student`.`adjusted_course_price`,`student`.`admission_date`,`student`.`id` as `student_id`,`student`.`roll` as `roll`, `student`.`first_name`, `student`.`gender`, `student`.`adjusted_course_price` as `amount`, `subject`.`name` FROM `student` LEFT JOIN
        `subject` ON `subject`.`id` = `student`.`subject_id`  WHERE
    ...
    public function getInvoiceBasic($studentID = '')
    {
        ...
        $this->db->select('s.id,s.roll,s.branch_id,sub.fee,sub.name as subject_name,s.course_price,s.course_price_discount,s.adjusted_course_price, s.first_name,s.last_name,s.email as student_email,s.current_address as student_address,c.name as class_name,b.school_name,b.email as school_email,b.mobileno as school_mobileno,b.address as school_address');
    ...
    public function getStudentFeeDeposit($student_id, $financial_year_id)
    {
        $sqlDeposit = "SELECT IFNULL(SUM(`amount`), '0.00') as `total_amount`, IFNULL(SUM(`discount`), '0.00') as `total_discount`, IFNULL(SUM(`fine`), '0.00') as `total_fine` FROM `fee_payment_history` WHERE `student_id` = " . $this->db->escape($student_id) . "";
        return $this->db->query($sqlDeposit)->row_array();
    }
    public function getStudentFeeDepositThroughOfficeAccount($student_id)
    {
        $sqlDeposit = "SELECT IFNULL(SUM(`amount`), '0.00') as `total_amount` FROM `transactions` WHERE `type`='deposit' AND `roll` = " . $this->db->escape($student_id) . "";
        return $this->db->query($sqlDeposit)->row_array();
    }
    ...
    public function getBatchWisePaymentReport($classID = '', $sectionID = '', $studentID = '', $branchID = '')
    {
        ...
        s.register_no, 
        s.course_price, 
        s.adjusted_course_price, 
        s.course_price_discount, 
        COALESCE(SUM(h.amount), 0) as total_amount
    ');
        $this->db->from('student as s');
        $this->db->join('fee_payment_history as h', 'h.student_id = s.id', 'left');
    ...
    public function saveTransaction($data = array())
    {
        ...
        $arrayHead = array(
            ...
            'type' => 'income',
            'system' => 1,
            'branch/* The above code is a comment block in PHP. Comments in PHP are used to add
            explanations or notes within the code that are not executed by the PHP engine. In
            this case, the comment block is using the multi-line comment syntax `/* */` to
            provide a comment that spans multiple lines. The text "basic" is included within
            the comment block. Additionally, the ` */
            _id' => $branchID,
        );
