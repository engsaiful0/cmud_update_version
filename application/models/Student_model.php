...
class Student_model extends MY_Model
{
    ...
    public function update($data = array(), $getBranch = array(), $id = '')
    {
        ...
        $inser_data1 = array(
            ...
            'hostel_id' => $hostelID,
            'room_id' => $roomID,
            'name_of_medical_college' => $this->input->post('name_of_medical_college'),
            'previous_remarks' => $this->input->post('previous_remarks'),
            'photo' => $this->uploadImage('student'),
            'subject_id' => $this->input->post('subjects'),
            'course_price' => $this->input->post('course_price'),
            'course_price_discount' => $this->input->post('course_price_discount'),
            'adjusted_course_price' => $this->input->post('adjusted_course_price'),
            'nid_number' => $this->input->post('nid_number'),
            'batch' => $this->input->post('batch'),
            'class_id' => $this->input->post('class_id'),
            'is_online_offline' => $this->input->post('is_online_offline'),
            'year_of_admission_into_medical_college' => $this->input->post('year_of_admission_into_medical_college'),
            'year_of_passing_ssc' => $this->input->post('year_of_passing_ssc'),
            'year_of_passing_hsc' => $this->input->post('year_of_passing_hsc'),
            'year_of_passing_final_prof' => $this->input->post('year_of_passing_final_prof'),
            'one_year_internship_training' => $this->input->post('one_year_internship_training'),
            'bmdc_reg_no' => $this->input->post('bmdc_reg_no'),
            'valid_upto' => (isset($data['valid_upto']) ? date("Y-m-d", strtotime($data['valid_upto'])) : ""),
        );
        ...
        if (!isset($data['student_id']) && empty($data['student_id'])) {
            ...
            $this->db->where('id', $id)->update('student', $inser_data1);
    ...
    {
        $this->db->select('s.*,s.class_id,s.section_id,s.roll,s.branch_id,s.session_id,c.name as class_name,se.name as section_name');
    ...
    public function countAllStudents($roll = '', $branchID = '', $class_id = '', $subject_id = '')
    {
        // Select columns you need
        $this->db->select('s.*');
