...
class Fees_model extends MY_Model
{
    ...
    {
        $sql = "SELECT `student`.`register_no`,`student`.`financial_year_id`,`student`.`admission_date`,`student`.`id` as `student_id`, `student`.`first_name`, `student`.`gender`, `subject`.`fee` as `amount`, `subject`.`name` FROM `student` LEFT JOIN
    ...
    public function getInvoiceBasic($studentID = '')
    {
        ...
        $this->db->select('s.id,e.branch_id,sub.fee,sub.name as subject_name, s.first_name,s.last_name,s.email as student_email,s.current_address as student_address,c.name as class_name,b.school_name,b.email as school_email,b.mobileno as school_mobileno,b.address as school_address');
