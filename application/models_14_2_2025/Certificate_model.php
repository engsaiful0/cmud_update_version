...
class Certificate_model extends MY_Model
{
    ...
    public function get_name_by_bmdc_reg_no($bmdc_reg_no)
    {
        // Ensure the $bmdc_reg_no is properly sanitized
        $bmdc_reg_no = $this->security->xss_clean($bmdc_reg_no);

        // Query to fetch student information along with subject details
        $this->db->select('student.id as student_id,student.first_name, student.roll, subject.name as subject_name, subject.course_duration, subject.course_duration_years_month_day');
        $this->db->from('student');
        $this->db->join('subject', 'student.subject_id = subject.id', 'left');
        $this->db->where('student.bmdc_reg_no', $bmdc_reg_no);
        ...
        if ($query->num_rows() > 0) {
            ...
            return [
                ...
                'data' => [
                    ...
                    'course_duration' => $result['course_duration'] . ' ' . $result['course_duration_years_month_day'],
                ]
            ];
        } else {
            return [
                'success' => false,
                'message' => 'No data found for the given BMDC Reg No'
            ];
        }
    }
    ...
    public function getDataById($id)
    {
        ...
        $this->db->where('certificates_templete.id', $id);
        if (!is_superadmin_loggedin()) {
            $this->db->where('certificates_templete.branch_id', get_loggedin_branch_id());
        }
        $this->db->order_by('certificates_templete.id', 'ASC');
