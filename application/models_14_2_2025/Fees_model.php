...
class Fees_model extends MY_Model
{
    ...
    public function getInvoiceStatus($studentID = '')
    {
        ...
        if ($paid['amount'] == 0) {
            $status = 'unpaid';
        } elseif ($totalFee == ($paid['amount'] + $paid['discount'])) {
            $status = 'total';
    ...
    {
        $sql = "SELECT `student`.`register_no`,`student`.`financial_year_id`,`student`.`course_price`,`student`.`course_price_discount`,`student`.`adjusted_course_price`,`student`.`admission_date`,`student`.`id` as `student_id`, `student`.`first_name`, `student`.`gender`, `student`.`adjusted_course_price` as `amount`, `subject`.`name` FROM `student` LEFT JOIN
    ...
    public function getAllPayments($class_id = '', $student_id = '', $branch_id = '')
    {
    ...
    public function getInvoiceList($class_id = '', $student_id = '', $branch_id = '')
    {
        $this->db->select('s.id as student_id,sub.name as subject_name,s.first_name,s.last_name,s.register_no,s.mobileno,c.name as class_name');
