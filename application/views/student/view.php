<script type="text/javascript">
	...
	function studentQuickViewInStudentView(id) {
    $.ajax({
        url: base_url + 'student/quickDetails',
        type: 'POST',
        data: {
            student_id: id,
            '<?= $this->security->get_csrf_token_name(); ?>': '<?= $this->security->get_csrf_hash(); ?>' // CSRF Token added correctly
        },
        dataType: 'json',
	...
	function getClassByBranchStudentView(branchID) {
		$.ajax({
	...
	$(document).ready(function() {
		$("#search_by_roll").autocomplete({
			...
			select: function(event, ui) {
				$('#search_by_roll').val(ui.item.label);
				return false;
			}
		});
	});
				...
				<div class="row mb-sm">
					<?php if (is_superadmin_loggedin()): ?>
						<div class="col-md-2 mb-sm">
							<div class="form-group">
								...
								?>
								<select onchange='getClassByBranchStudentView(this.value)' class="form-control" id="branch_id" data-plugin-selecttwo name="branch_id">
					...
					<div class="col-md-2 mb-sm">
						<div class="form-group">
							...
							<select onchange='getStudentByBatch(this.value)' class="form-control" id="class_id" data-plugin-selecttwo name="class_id">
					...
					<div class="col-md-2 mb-sm">
						<div class="form-group">
							...
							<input placeholder="Enter Name" type="text" name="search_by_anything" id="search_by_anything" class="form-control">
						</div>
					</div>
					<div class="col-md-2 mb-sm">
						<div class="form-group">
							<label class="control-label">Student ID</label>
							<input placeholder="Type student ID.." type="text" name="roll" id="search_by_roll" class="form-control">
						</div>
					</div>
					<div class="col-md-2 mb-sm">
						...
						<tbody>
							...
							?>
								<tr>
									...
									<td class="action">
										<!-- quick view -->
										<a href="javascript:void(0);" onclick="studentQuickViewInStudentView('<?= $row['student_id'] ?>');" class="btn btn-default btn-circle icon" data-toggle="tooltip"
										...
										if (get_permission('student', 'is_delete')): ?>
											<!-- delete link -->
											<?php echo btn_delete('student/delete_data/' . $row['student_id'] . '/' . $row['student_id']); ?>
										<?php endif; ?>
									</td>
								</tr>
							<?php endforeach; ?>
					...
					swal({
						...
						if (result.value) {
							$.ajax({
								...
								dataType: "JSON",
								data: {
									array_id: arrayID // Fix: Pass arrayID correctly
								},
								success: function(data) {
									swal({
