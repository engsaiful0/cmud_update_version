(function ($) {
    $(function () {
        var form = $('#deposit-filters');
        var results = $('#deposit-results');
        var dates = $('#deposit-dates');
        var pending;
        var timer;
        var sequence = 0;
        ...
        function load(url) {
            ...
            $('#deposit-filter-error').hide();
            $('#deposit-filter-loading').show();
            ...
            pending = $.ajax({
                ...
                $('#deposit-filter-error').text(message || 'Unable to load deposits. Please try again.').show();
            }).always(function () {
                if (request !== sequence) return;
                $('#deposit-filter-loading').hide();
        ...
        form.on('change', 'select', function () {
            if (this.id === 'deposit-branch') $('#deposit-head').val('');
            load();
        });
        ...
        $('#deposit-reset').on('click', function () {
