(function ($) {
    $(function () {
        var form = $('#expense-filters');
        var results = $('#expense-results');
        var dates = $('#expense-dates');
        var pending;
        var timer;
        var sequence = 0;
        ...
        function load(url) {
            ...
            $('#expense-filter-error').hide();
            $('#expense-filter-loading').show();
            ...
            pending = $.ajax({
                ...
                $('#expense-filter-error').text(message || 'Unable to load expenses. Please try again.').show();
            }).always(function () {
                if (request !== sequence) return;
                $('#expense-filter-loading').hide();
        ...
        form.on('change', 'select', function () {
            if (this.id === 'expense-branch') $('#expense-head').val('');
            load();
        });
        ...
        $('#expense-reset').on('click', function () {
